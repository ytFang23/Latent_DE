...
def compute_loss_all_batches(model,
    ...
    """
    Fully-torch evaluation.

    - PhysioNet: binary ROC AUC using a torch implementation.
    - Activity: accuracy + mean/std/var across Monte-Carlo samples S,
                using torch only, with correct batch-wise concatenation.
    """

    # ---------- helpers (torch-only) ----------
    def _roc_auc_score_binary_torch(y_true: torch.Tensor, y_score: torch.Tensor) -> torch.Tensor:
        """y_true, y_score are 1D tensors on the same device."""
    ...
    # ---------- per-batch collectors for classification ----------
    # We collect per-batch tensors then concat once at the end.
    preds_batches = []   # list of logits per batch, shaped (S, Bflat_batch, C)
    labs_batches  = []   # list of labels per batch, shaped (Bflat_batch, C)
    ...
    for i in range(n_batches):
        ...
        if args.classif and ("label_predictions" in results):
            logits = results["label_predictions"]              # (S,B,C) or (S,B,T,C)
            # ---- reshape logits to (S, Bflat_batch, C) ----
            if logits.dim() == 4:                              # (S, B, T, C)
                S, B, T, C = logits.shape
                logits = logits.reshape(S, B * T, C)
            elif logits.dim() == 3:                            # (S, B, C)
                S, B, C = logits.shape
                logits = logits.reshape(S, B, C)
            else:
                # fallback: at least force (S,B,C)
                logits = logits.reshape(1, -1, model.n_labels)

            labs = batch_dict["labels"]
            # ---- reshape labels to (Bflat_batch, C) ----
            if labs.dim() == 3:                                # (B, T, C)
    ...
    if args.classif and len(preds_batches) > 0:
        dirname = os.path.join("plots", str(experimentID))
        os.makedirs(dirname, exist_ok=True)

        # Concatenate across batches (so Bflat matches!)
        preds_all = torch.cat(preds_batches, dim=1)  # (S, sum_Bflat, C)
        labs_all  = torch.cat(labs_batches,  dim=0)  # (sum_Bflat, C)
        S = preds_all.size(0)
        C = preds_all.size(-1)

        if args.dataset == "physionet":
            # Binary sequence-level ROC AUC (torch-only)
            labs = _squeeze_last(labs_all)          # (sum_Bflat,) or (sum_Bflat,C)
            if labs.dim() > 1:
                labs = labs[:, 0]                   # choose first/only col for binary

            # Choose the positive class score
            scores = preds_all[..., 0] if C == 1 else preds_all[..., -1]  # (S, sum_Bflat)

            # If you want to average S Monte-Carlo AUCs, do it here; many works compute on S=1.
            # We compute on the mean score across S for stability:
            scores_mean = scores.mean(dim=0)        # (sum_Bflat,)
            valid = ~torch.isnan(labs)
            labs_v = labs[valid]
            scores_v = scores_mean[valid]
            if labs_v.numel() > 0 and torch.unique(labs_v).numel() >= 2:
                auc = _roc_auc_score_binary_torch(labs_v, scores_v)
                total["auc"] = float(auc)
            else:
                total["auc"] = 0.0

        elif args.dataset == "activity":
            # Accuracy across labeled time steps with Monte-Carlo samples.
            # preds_all: (S, N*T_total, C)
            # labs_all:  (N*T_total, C)
            valid_mask = (labs_all.sum(dim=-1) > 0)          # (N*T_total,)
            if valid_mask.any():
                labs_sel  = labs_all[valid_mask, :]          # (M, C)
                preds_sel = preds_all[:, valid_mask, :]      # (S, M, C)

                class_labels  = labs_sel.argmax(dim=-1)      # (M,)
                pred_class_id = preds_sel.argmax(dim=-1)     # (S, M)

                eq = (pred_class_id == class_labels.unsqueeze(0)).float()  # (S, M)
                acc_per_s = eq.mean(dim=1)                                  # (S,)
                ...
                total["accuracy_std"] = torch.tensor(0.0, device=device)
                total["accuracy_var"] = torch.tensor(0.0, device=device)

    # ensure torch tensors on correct device for all totals
