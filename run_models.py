	...
	for itr in range(1, num_batches * (args.niters + 1)):
		...
		if itr % (n_iters_to_viz * num_batches) == 0:
			with torch.no_grad():
				...
				# helper: turn tensor/number -> float for safe logging/formatting
				def _scalar(x):
					try:
				...
				message = 'Epoch {:04d} [Test seq (cond on sampled tp)] | Loss {:.6f} | Likelihood {:.6f} | KL fp {:.4f} | FP STD {:.4f}|'.format(
					itr // num_batches,
					_scalar(test_res["loss"]), _scalar(test_res["likelihood"]),
					_scalar(test_res["kl_first_p"]), _scalar(test_res["std_first_p"])
				)

				logger.info("Experiment " + str(experimentID))
				logger.info(message)
				logger.info("KL coef: {}".format(kl_coef))
				logger.info("Train loss (one batch): {}".format(_scalar(train_res["loss"])))
				logger.info("Train CE loss (one batch): {}".format(_scalar(train_res["ce_loss"])))

				if "auc" in test_res:
					logger.info("Classification AUC (TEST): {:.4f}".format(_scalar(test_res["auc"])))

				if "mse" in test_res:
					logger.info("Test MSE: {:.4f}".format(_scalar(test_res["mse"])))

				if "accuracy" in train_res:
					logger.info("Classification accuracy (TRAIN): {:.4f}".format(_scalar(train_res["accuracy"])))

				# ---- added: accuracy mean/std/var on TEST ----
				if "accuracy" in test_res:
					logger.info("Classification accuracy (TEST): {:.4f}".format(_scalar(test_res["accuracy"])))
				if "accuracy_std" in test_res:
					logger.info("Classification accuracy STD (TEST): {:.6f}".format(_scalar(test_res["accuracy_std"])))
				if "accuracy_var" in test_res:
					logger.info("Classification accuracy VAR (TEST): {:.6f}".format(_scalar(test_res["accuracy_var"])))
				# ----------------------------------------------

				if "pois_likelihood" in test_res:
					logger.info("Poisson likelihood: {}".format(_scalar(test_res["pois_likelihood"])))

				if "ce_loss" in test_res:
					logger.info("CE loss: {}".format(_scalar(test_res["ce_loss"])))
